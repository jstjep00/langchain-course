...
@traceable
def template_creation() -> PromptTemplate:
    summary_template = """
         given the information {information} I want you to create a:
         1. short summary about the person
...
@traceable
def llm_creation() -> VLLMOpenAI:
    llm = VLLMOpenAI(
        openai_api_key="EMPTY",
        openai_api_base=f"{os.getenv("OPENAI_API_BASE")}",
        model_name=f"{os.getenv("MODEL_NAME")}",
        max_tokens=1024,
    )
    return llm
...
"""
    ...
    summary_prompt_template = template_creation()
